...
/**
 * 
 * Custom React hook to fetch weekly forecast data from OpenWeatherMap API
 * 
 * @param {WeatherOptions} options - Configuration object for weekly forecast request
 * 
 * @returns {Object|null} Weekly forecast data object from the API, or null while loading
 * 
 * @example
 * // Using city name
 * const weeklyForecast = getWeeklyForecast({ city: "Copenhagen" });
 * 
 * @example
 * // Using coordinates with custom language and units
 ...
 * });
 */
export function getWeeklyForecast({ longitude, latitude, units = "metric", lang = "da" }) {
    ...
    useEffect(() => {
        ...
        const url = `${BASE_URL}/onecall?lat=${latitude}&lon=${longitude}&exclude=current,minutely,hourly,alerts&units=${units}&lang=${lang}&appid=${API_KEY}`;
    ...
    }, [longitude, latitude, lang, units]);

    return { weeklyForecast, loading, error };
}